...
class TestAmbition(testLib.RestTestCase):
	...
	responseData = self.makeRequest("/users/getUserInfo", method="GET", data={'user':'1'})
	self.assertDictEqual({'Code':testLib.RestTestCase.SUCCESS,'data':{'user':'1', 'spScore':1, 'mpScore':2,'balance':0}}, responseData )
    #save a lower score and then save a higher score for the same user, see if we update the db correctly by take the higher score for that user
    def testOverwriteSingleScore(self):
	self.makeRequest("/users/SaveScores/single", method="POST", data={'user':'1', 'score':1})
	self.makeRequest("/users/SaveScores/single", method="POST", data={'user':'1', 'score':2})
	responseData = self.makeRequest("/users/getUserInfo", method="GET", data={'user':'1'})
	self.assertDictEqual({'Code':testLib.RestTestCase.SUCCESS, 'data':{'user':'1', 'spScore':2, 'mpScore':0}}, self.extractDict(responseData) )
	...
	responseData = self.makeRequest("/users/getUserInfo", method="GET", data={'user':'1'})
	self.assertDictEqual({'Code':testLib.RestTestCase.SUCCESS, 'data': {'user':'1', 'spScore':13, 'mpScore':12}}, self.extractDict(responseData) )
	...
	respData = self.makeRequest("/users/Top10Scores/single", method="GET",data={})
	...
	respData = self.makeRequest("/users/Top10Scores/multiple", method="GET",data={})
	self.assertDictEqual({'Code':testLib.RestTestCase.SUCCESS,'data':[{'user':'12', 'score':12},{'user':'11', 'score':11},{'user':'10', 'score':10},{'user':'9', 'score':9},{'user':'8', 'score':8},{'user':'7', 'score':7},{'user':'6', 'score':6},{'user':'5', 'score':5},{'user':'4', 'score':4},{'user':'3', 'score':3}]},respData)
